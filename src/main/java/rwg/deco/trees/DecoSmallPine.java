...
public class DecoSmallPine extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (i = 0; i < startHeight; i++) {
            world.setBlock(x, y, z, Block.wood.blockID, metadata, 0);
            y++;
        }
    ...
    public void buildBranch(World world, Random rand, int x, int y, int z, int dX, int dZ, int logLength,
        ...
        for (int m = 1; m <= logLength; m++) {
            world.setBlock(x + (dX * m), y, z + (dZ * m), Block.wood.blockID, metadata, 0);
        }
    }
    ...
    public void buildLeaves(World world, int x, int y, int z) {
        ...
        if (b == null) {
            world.setBlock(x, y, z, Block.leaves.blockID, metadata, 0);
        }
    }
}

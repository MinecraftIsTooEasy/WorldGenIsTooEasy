...
public class DecoMangrove extends WorldGenerator {
    ...
    /*
     * Block.wood, 0, Block.leaves, 0, 9 + rand.nextInt(5), 3 + rand.nextInt(2), 13f, 3, 0.32f, 0.1f
     */

    public DecoMangrove(Block log, int metaLog, Block leaves, int metaLeaves, int baseHeight, int rootHeight,
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        if (b != Block.grass && b != Block.dirt && b != Block.sand) {
            if (!(b == Block.waterStill && world.getBlock(x, y - 2, z) == Block.sand
        ...
        for (int i = y + root; i < y + base; i++) {
            world.setBlock(x, i, z, blockLog.blockID, metadataLog, 2);
        }
    ...
    public void generateBranch(World world, Random rand, float x, float y, float z, double horDir, float verDir,
        ...
        while (c < length) {
            world.setBlock((int) x, (int) y, (int) z, blockLog.blockID, metadataLog, 2);
            ...
            for (j = -s; j <= s; j++) {
                for (k = -w; k <= w; k++) {
                    ...
                    if (dist <= size - 0.5f || (dist <= size && rand.nextBoolean())) {
                        if (dist < 0.6f) {
                            world.setBlock(x + i, y + j, z + k, blockLog.blockID, metadataLog, 2);
                        }
                        if (world.isAirBlock(x + i, y + j, z + k)) {
                            world.setBlock(x + i, y + j, z + k, blockLeaves.blockID, metadataLeaves, 2);
                        }
                    }
                }
            }

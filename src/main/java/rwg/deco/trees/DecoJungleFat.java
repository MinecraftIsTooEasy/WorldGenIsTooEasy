...
public class DecoJungleFat extends WorldGenerator {
    ...
    /*
     * Block.wood, 0, Block.leaves, 0, 18 + rand.nextInt(5), 5 + rand.nextInt(2), 16f, 5, 0.32f, 0.1f
     */
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = y + root - 2; i < y + base + 2; i++) {
            ...
            world.setBlock(x + 1, i, z + 1, blockLog.blockID, metadataLog, 2);
        }
        ...
        for (int j = 0; j < branch; j++) {
            ...
            for (int m = 0; m < 2; m++) {
                generateLeaves(
                        world,
                        rand,
                        x + eX - 2 + rand.nextInt(5),
                        y + base + eY + rand.nextInt(2),
                        z + eZ - 2 + rand.nextInt(5),
                        4f,
                        1.5f);
            }
            ...
            for (j = -s + 1; j <= s; j++) {
                for (k = -w; k <= w; k++) {
                    ...
                    if (dist <= size - 0.5f || (dist <= size && rand.nextBoolean())) {
                        if (dist < 1.3f) {
                            world.setBlock((int) x + i, (int) y + j, (int) z + k, blockLog.blockID, metadataLog, 2);
                        }
                        if (world.isAirBlock((int) x + i, (int) y + j, (int) z + k)) {
                            world.setBlock((int) x + i, (int) y + j, (int) z + k, blockLeaves.blockID, metadataLeaves, 2);
                        }
                    }
                }
            }

...
public class DecoWillow extends WorldGenerator {
    ...
    public boolean generate(World world, Random rand, int x, int y, int z) {
        ...
        for (int b = 0; b < branches; b++) {
            ...
            while (c < branchLenght) {
                ...
                c++;
                hd += 0.5f;
                world.setBlock(x + (int) (c * xd), y + (int) hd, z + (int) (c * yd), Block.wood.blockID, 12, 0);
            }
            createLeavesAroundBranch(world, rand, x + (int) (c * xd), y + (int) hd, z + (int) (c * yd), 2, 1);
        }
                ...
                for (int k = -s; k <= s; k++) {
                    ...
                    if (l <= t) {
                        if (world.isAirBlock(x + i, y + j, z + k) && (l < t - c || rand.nextBoolean())) {
                            world.setBlock(x + i, y + j, z + k, Block.leaves.blockID, 6, 0);
    ...
    private void createVine(World world, Random rand, int x, int y, int z) {
        ...
        for (int i = -1; i > -r; i--) {
            ...
            world.setBlock(x, y + i, z, Block.leaves.blockID, 6, 0);
        }
    }

...
public class RealisticBiomeCoastIce extends RealisticBiomeBase {
    ...
    public RealisticBiomeCoastIce() {
        super(0, RWGBaseBiomes.baseSnowDesert);

        surface = new SurfaceGrassland(Block.ice, Block.ice, Block.ice, Block.blockSnow);
    }
        ...
        for (int k = 255; k > -1; k--) {
            ...
            if (b == null) {
                ...
                if (cliff) {
                    ...
                    if (depth < 10) {
                        blocks[(y * 16 + x) * 256 + k] = type == 0 ? Block.ice : Block.blockSnow;
                    }
                } else {
                    if (depth < 5 && k > 61) {

...
public class RealisticBiomeSnowHills extends RealisticBiomeBase {
    ...
    public RealisticBiomeSnowHills() {
        super(0, RWGBaseBiomes.baseSnowForest, RWGBiomes.coastIce, RWGBaseBiomes.baseRiverIce);

        terrain = new TerrainMountainSpikes();
        surface = new SurfaceMountainSnow(Block.grass, Block.dirt, false, null, 0.2f);
    }
    ...
    public void rDecorate(World world, Random rand, int chunkX, int chunkY, NoiseGenerator perlin, CellNoise cell,
        ...
        for (int b1 = 0; b1 < l * 2f * strength; b1++) {
            ...
            if (z52 < 75) {
                WorldGenerator worldgenerator = rand.nextInt(8) != 0 ? new DecoSmallSpruce(1 + rand.nextInt(2))
                        : new DecoSmallPine(1 + rand.nextInt(3), 2 + rand.nextInt(4));
                worldgenerator.setScale(1.0D, 1.0D, 1.0D);
                worldgenerator.generate(world, rand, j6, z52, k10);
            } else if (z52 < 110) {
                WorldGenerator worldgenerator = rand.nextInt(4) != 0 ? new DecoSmallSpruce(rand.nextInt(2))

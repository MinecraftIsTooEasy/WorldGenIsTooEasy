...
public class MapGenAncientVillage {
    ...
    public void build(World world, Block[] block, int dis) {
        int y = 120;
        for (; y > 49; y--) {
            if (block[coordstoArray(8, y - 1, 8)] != null) {
                if (y > 118) {
                    return;
                }
                break;
        ...
        for (i = startX; i < startX + sizeX; i++) {
            if (block[coordstoArray(startX + i, y - 2, startZ)] != null) {
                height = rand.nextInt(4);
                for (j = 0; j < height; j++) {
                    block[coordstoArray(startX + i, y + j - 1, startZ)] = Block.cobblestoneMossy;
                }
            }

            if (block[coordstoArray(startX + i, y - 2, startZ + sizeZ)] != null) {
                height = rand.nextInt(4);
                for (j = 0; j < height; j++) {
                    block[coordstoArray(startX + i, y + j - 1, startZ + sizeZ)] = Block.cobblestoneMossy;
                }
            }
        }

        for (i = startZ + 1; i < startZ + sizeZ - 1; i++) {
            if (block[coordstoArray(startX, y - 2, startZ + i)] != null) {
                height = rand.nextInt(4);
                for (j = 0; j < height; j++) {
                    block[coordstoArray(startX, y + j - 1, startZ + i)] = Block.cobblestoneMossy;
                }
            }

            if (block[coordstoArray(startX + sizeX, y - 2, startZ + i)] != null) {
                height = rand.nextInt(4);
                for (j = 0; j < height; j++) {
                    block[coordstoArray(startX + sizeX, y + j - 1, startZ + i)] = Block.cobblestoneMossy;
                }
            }
        }
    }
